...
class TelemetryEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if action == 0:
            ...
            # action_edge_index = [i for i, l in enumerate(self.edge_port_index) if action in l][0]   
        ...
        reward = self.clc_rwd_cost(action+1)
    ...
    def handle_probe(self, probe):  
        # for idx, probex in enumerate(probe):
        #     probe[idx] = probex[0::2]+[probex[-1]]
        # return probe
        probe_dp = []
        for probex in probe:
...
if __name__ == "__main__":
    ...
    env.step(19)
    env.step(12)
    port_req = [[4, 1, 2, 3, 5], [4, 3, 5, 1], [3, 2, 1], [5, 2, 4, 1], [3, 4], [3, 4], [5, 2, 3, 4], [5, 3, 1, 4, 2], [1, 4, 2, 5, 3], [2, 5], [2, 4, 1], [3, 4], [3, 5], [5, 3], [5, 3, 2]]
    probe =[[2, 15, 14, 16, 11, 12, 10, 18, 17, 19], [13, 28, 9, 27, 4, 8, 3, 6, 0, 5, 1, 22, 21, 24], [7, 26, 25, 29, 20, 23]]
    overheads = env.overheads(port_req=port_req, probe=probe)
    data_my = probe_data(probe)
    probe_dp = env.handle_probe(probe) 
    print(probe_dp, overheads)
    print(data_my)
